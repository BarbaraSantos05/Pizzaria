...
Namespace ClassesGenerica
    Public Class Funcoes
        ...
        Public Shared Sub FechaFormulario(ByVal nomeFormulario As String)
            ...
            For Each frm As Form In CType(MdiPizzaria, System.Windows.Forms.Form).MdiChildren
        ...
        Private Function ListaItensCombo(ByRef con As Connection, ByVal tabela As String, ByVal id As String, ByVal nome As String) As System.Data.DataTable

            Try

                con.Command.Parameters.Clear()

                con.Command.CommandText = "Select " + id + " as codigo, " + nome + " as nome" +
                    " From " + tabela +
                    " Order by " + nome
        ...
        Public Sub PreencheCombo(ByRef combo As ComboBox, ByVal tabela As String, ByVal id As String, ByVal nome As String)
            ...
            Try
                ...
                Dim dt = ListaItensCombo(con, tabela, id, nome)

                con.CloseConnection()

                If dt.Rows.Count > 0 Then

                    For i As UShort = 0 To dt.Rows.Count - 1

                        Dim item = New Item(dt.Rows(i).Item("codigo"), dt.Rows(i).Item("nome"))
        ...
        Public Sub BuscaIndex(ByVal obj As ComboBox, Optional ByVal codigo As Int64 = -1, Optional ByVal texto As String = "")

            Try

                obj.SelectedIndex = -1

                For I As Int32 = 0 To obj.Items.Count - 1
                    'OBJ.SelectedIndex = I
                    If codigo <> -1 And obj.Items(I).ToString <> "" Then
                        '***** BUSCA POR CÓDIGO *****
                        If CType(obj.Items(I), Item).Codigo = codigo Then
                            obj.SelectedIndex = I
                            Exit For
                        End If
                    ElseIf codigo = -1 And texto.Trim <> "" Then
                        '***** BUSCA POR TEXTO *****
                        If CorrigeString(obj.Items.Item(I).ToString.ToUpper.Trim) = CorrigeString(texto.ToUpper.Trim) Then
                            obj.SelectedIndex = I
                            Exit For
                        End If
                    End If
                Next
            Catch erro As Exception
                'Call ErroCritico(Err.Number, erro.Message, "Funcoes - BuscaIndex")
            End Try

        End Sub

        Public Function PegaIdCombo(ByRef combo As ComboBox) As Int64

            Dim id As Int64 = 0

            If combo.Text <> "" Then
                Try
                    id = CType(combo.Items(combo.SelectedIndex), Item).Codigo
                Catch ex As Exception
                    id = 0
                End Try
            End If
